...
\todo{Add something like ``we revisit'' since this is already in intro/background chapters}

\textit{Quantum nodes}.
A quantum internet connects quantum nodes on which quantum programs may be 
executed.
%(Figure~\ref{qoala:fig:qoala_location}). 
...
Multiple programs, each running on their own node, together form an \textit{application} (see~\ref{qoala:fig:program_illustration}), e.g. QKD (two programs, one per node),
...
%    \centering
    ...
    %}
    %\label{qoala:fig:qoala_location}
%\end{figure}
