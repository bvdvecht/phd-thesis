...
Based on these design considerations, we propose Qoala (see Fig.~\ref{qoala:fig:runtime_overview}), an execution environment for programmable nodes in a quantum internet. 
...
Qoala is based on only a few core assumptions on the processing node (Figure~\ref{qoala:fig:minimal_hardware_assumptions}):
...
Figure~\ref{qoala:fig:example_program} shows an example program in text format.
...
the runtime provides \textit{classical sockets} and \textit{EPR sockets} based on~\cite{dahlberg2022netqasm}. Host code uses classical sockets for sending and receiving messages; EPR sockets are indicated in request routines (see e.g. Figure~\ref{qoala:fig:example_program}).
...
\begin{figure*}
    ...
    \centering
    \subfloat[\centering \label{qoala:fig:task_types}]{{\includegraphics[height=\figheight, keepaspectratio]{figures/qoala/task-types-table.png}}}%
    \hfill
    \subfloat[\centering \label{qoala:fig:task_creation_cutout}]{{\includegraphics[height=\figheight, keepaspectratio]{figures/qoala/task_creation_cutout.png}}}%
    \caption{
...
Tasks have different types (Figure~\ref{qoala:fig:tasks}) based on the code they represent.
...
See Figure~\ref{qoala:fig:task_types} for task types and Figure~\ref{qoala:fig:task_creation_cutout} for examples of host code and corresponding tasks (details in Appendix~\ref{qoala:sec:app:scheduling_execution}).


\subsection{Program instantiation}
\label{qoala:sec:program_instantiation}
A program is part of an application that uses entanglement generation orchestrated by a network controller (Figure~\ref{qoala:fig:program_illustration}).
