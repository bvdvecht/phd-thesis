...
This section provides more details about tasks, task creation and scheduling (Section~\ref{qoala:sec:architecture}) as well as about our scheduler implementation~\ref{qoala:sec:implementation}.
...
\end{itemize}
Figure~\ref{qoala:fig:app:task_creation} shows an overview of blocks and corresponding tasks and their precedence constraints.
...
Figure~\ref{qoala:fig:app:linear_non_linear} illustrates the difference between predictable and unpredictable programs.
...
\textbf{\texttt{SinglePair} task execution.} A \texttt{SinglePair} task is executed by the QPS. First, arguments are read from shared memory. Then, an EPR request (see Appendix~\ref{qoala:sec:app:entanglement_distribution}) is sent to the network controller.
...
In this and the following sections we describe the scheduler from our implementation (Section~\ref{qoala:sec:implementation}).
...
Edges in the node scheduler graph between heterogenous tasks (i.e. between CPS and QPS tasks) are represented in the partial processor graphs by the \textit{external dependencies} node attribute. See Figure~\ref{qoala:fig:app:task_graph_partial} for an example.
...
Here we describe how the schedulers communicate in our implementation (Section~\ref{qoala:sec:implementation}).
...
Below we describe the high-level steps involved in the node scheduler algorithm implementation of Section~\ref{qoala:sec:implementation}.
\label{qoala:sec:app:node_scheduler_algorithm}
\begin{enumerate}
    \item Split the current task graph into a partial CPS graph and a partial QPS graph. For the algorithm, see Appendix~\ref{qoala:sec:app:task_graph_splitting}.
    ...
    \item If the finished task was a \texttt{HostLocal} task for some program instance P, and if the CPS partial graph is empty, check which block the program instance should jump to. This information is given by the task itself (and stored in the completed task list of the CPS scheduler), after evaluating the last instruction (a jump or branch instruction) in the BB that the task represented. For this new BB, create corresponding tasks for both the CPS and QPS. Task creation is discussed in Appendix~\ref{qoala:sec:app:scheduling_execution_task_creation}.
...
Below we describe the high-level steps involved in the CPS scheduler algorithm implementation of Section~\ref{qoala:sec:implementation}.
\begin{enumerate}
...
Qoala does not care whether this controller is implemented as a single entity, or is distributed in some way across multiple (processing) nodes (Figure~\ref{qoala:fig:app:network_controller_types}).
...
Populating the network controller's time slot with sessions is the result of (1) demand registration by nodes in the network, followed by (2) network schedule generation by the network controller itself, which we do not consider here (Figure~\ref{qoala:fig:app:network_controller_setup}).
...
The network stack and controller may be implemented in various ways, such as illustrated in Figure~\ref{qoala:fig:app:network_controller_types}.


\subsection{Detailed runtime overview}
Figure~\ref{qoala:fig:app:runtime_detailed} provides an overview of the runtime architecture.