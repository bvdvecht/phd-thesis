...
For optimization, the metrics we are interested in are makespan and application success probability. \todo{what to do with this sentence?}
...
\todo{example figure, or refer to earlier fig if exists}
...
Specifically, this runtime scheduler affect both the makespan (time it takes to execute the whole program) and the average application success probability (see \cref{chp:background} for metrics \todo{check this}).
...
\begin{figure}[t]
    ...
    QoalaHIR has quantum values as native types, which enables the compiler to do (quantum) value-use analysis in order to apply existing optimizations that make use of these value-use chains \todo{explain this in text}. Control-flow may be complex.
...
For QoalaHIR, use value-based representations.
...
This target includes at least the qubit topology of the quantum memory, the NetQASM flavor (i.e. the allowed quantum instructions, see also \cref{netqasm:sec:design_decisions_language}).
This information can come from the EHI.
We note that the EHI also contains information about noise and durations (of gates and network operations).
...
As mentioned above, the compiler's target comes from the EHI, which contains information about noise and durations (of gates and network operations).
...
It can use the EHI for this, which contains information about gate duration, estimated network operation duration, and coherence times of qubits.
