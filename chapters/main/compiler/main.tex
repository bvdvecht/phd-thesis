...
Having to deal with low-level details about how to express looping behavior, which and how much (quantum) memory to allocate, and how to integrate the classical and quantum code
...
In \cref{chp:netqasm,chp:qnodeos} we described how NetQASM and QNodeOS already provide a way for programmers to express program logic in a high-level language, Python.
...
NetQIR~\cite{vazquez-perez_netqir_2024} (an extension to QIR for network-related programs), and InQuIR~\cite{nishio_inquir_2023}. However, these are also for distributed quantum computing, and hence not for quantum networking which we are dealing with in this work.
...
In the context of networked programs, one could also say that correctness should include adherence to a (communication) protocol.
...
Also, entanglement generation may take milliseconds (\cref{qnodeos:fig:delcomp-epr-rate} in \cref{qnodeos:sec:processing_time_latencies}).
By contrast, individual local quantum operations like gates take only microseconds (\cref{tab:gate_durations} in \cref{qnodeos:sec:processing_time_latencies}) and local classical operations can be as fast as nanoseconds.
...
Because of the comparably long duration of network operations, special attention must be made by the compiler to minimize the time that there is quantum memory alive while doing network operations.
...
Even with optimizations (like re-ordering local operations) to minimize the time that quantum memory must remain alive, a network operation (such as entanglement generation) may take too long --- resulting in a too low-quality quantum memory to produce meaningful results.
This raises two points:
(1) the compiler can use Qoala's feature of \emph{deadlines}: by adding (relative) deadlines to (parts of) the code, the compiler can hint to the scheduler to, at runtime, either abort the program or to retry a certain piece of code, and
...
Specifically, the network schedule and the node scheduler affect both the makespan (time it takes to execute the whole program) and the average application success probability (see \cref{chp:background} for metrics).
...
\begin{figure}[t]
    ...
    Overall, the same structure is used as in \cref{compiler:fig:frontend-backend}, but now with three \ac{IR}s.
...
For classical code, existing optimization can be used from MLIR (part of LLVM), applied on the program when it is in one of the IRs (\cref{compiler:fig:overall-design}).
...
Moreover, by using the hybrid classical-quantum format of QoalaHIR we can re-use existing optimizations such as loop unrolling and others, see~\cite{mccaskey_mlir_2021, ittah_enabling_2022, nguyen_retargetable_2022, peduri_qssa_2022}.
...
\begin{inlinelist}
\item amount of CPS-QPS communication, since this negatively impacts execution time and possibly the time that quantum states must remain in memory, and
...
Although the compiler does not control this fidelity directly, it can indirectly help by doing clever capability negotiation (see \cref{chp:qoala}) and adding deadlines.
...
Existing quantum circuit optimizations can be applied to local quantum code.
...
\begin{itemize}
  ...
  \item mapping to specific qubits with better topology, and
...
q.measure()
  ...
  $q$ and $e$ are completely independent in terms of application logic.
  Since the creation of the entangled qubit $e$ may take a long time, executing the operations in the order they are given means that $q$ ---after the first Hadamard --- must remain in memory while waiting for the entanglement generation to finish.
