...
Again, we can re-use existing things. \todo{refs to noise-aware circuit optimizers}.
...
A protocol description can also help the compiler make more informed optimziations.
...
Many compilers and programming frameworks exist. \todo{refs}

Fully quantum circuit optmizers: \todo{refs}.
Noise-aware optmizers: \todo{refs}.
Hybrid classical-quantum compilers: \todo{refs}.

The idea of using IRs for quantum compilers has been used a lot: \todo{refs}.

Using MLIR in order to leverage existing compilation strategies has been investigated: \todo{refs}.

Compilers for quantum networking have not been studied much.
For distribution quantum computing there are compilers: \todo{refs}.

Session types and model checkers have been used for checking adherence to protocols: \todo{refs}.

QIR is a upcoming standard as an IR for qunatum computing. \todo{refs}
Recently there is network-related variant: NetQIR \todo{refs}.
...
Session types is a formalism used for defining communication protocols between different parties.
...
Tools exist that can check at compile-time if a program adhers to a communication protocol.

Session types may be used for two parties, but multi-party session types also exist.
Timed session types add a time element.
Session types have been investigated for use in the context of quantum protocols.

\section{Recommendations for futher research and evaluation}
