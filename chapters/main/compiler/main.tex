...
Compilers for classical computers have been developed and used since the creation of the first digital computers and are therefore a well-studied topic~\cite{aho_compilers_2006}.\steph{I would expect a specific reference beyond [1] for this statement}
...
Code optimization techniques include \steph{again I would expect more specific references to where this was first proposed maybe then just [1]}
\begin{itemize}
...
Benchmarks exist \steph{citation needed} with which to compare compilers, by looking at the output, like circuit size.
...
For distributed quantum computing, which is similar \steph{similar to what? I do btw not at all think it is similar given it optimizes a single program including also the distribution over processsors?}, compilers do exist~\cite{ferrari_compiler_2021, chatterjee_qurzon_2022, cuomo_optimized_2023, ferrari_modular_2023}.
...
an extension to QIR for network-related programs, NetQIR \steph{the way its written one doesnt understand that netqir is also for distributed comp}, has been proposed~\cite{vazquez-perez_netqir_2024}, and InQuIR~\cite{nishio_inquir_2023} for distributed quantum computing. \steph{There is some easy but important explanation missing here on why they do not already solve the problem}


\subsection{Session types}
\label{compiler:sec:session-types}
\steph{the jump from many subsxection on compilers to session types is a bit...aprupt. maybe expand the title of this or say "Other background: Session types" or so. In addition, say why we are talking about this now as its non obvius for the reader}
...
The \acf{EHI} (\cref{qoala:sec:runtime_environment}) can serve as the target for a compiler. \steph{asvice produced by compiler?}
...
We recommend using the existing technique of IRs, gradually lowering code \steph{lowering code is not well explained} and performing optimizations at each level.
...
\begin{itemize}
  ...
  By using value-semantics, the compiler can perform (quantum) data-flow analysis in order to apply classical optimization techniques such as loop unrolling or dead code elimination~\cite{peduri_qssa_2022, ittah_enabling_2022}. \steph{you had another figure on your slides at some point more detailed than figure 6.3 above - maybe helpful here?}
...
The NetQASM SDK (\cref{chp:netqasm} and \cref{chp:qnodeos}) may be used. \steph{say what language that is}
...
\steph{please add a section on implementation - even without evaluation that explains what was implemented - even if its just 1 pages succint with a link to tud gitlab or so,  this is great work already, and is very good to mention even without the evaluation}
