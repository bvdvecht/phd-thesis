...
As also explained in \cref{chp:background}, quantum network \textit{applications}, also called \textit{protocols}, are multi-partite programs that involve entanglement generation and classical communication between different end-nodes, as well as local computation.
...
These programs, running on different nodes, are in principle programmed and executed by independent actors (such as a client that programs and executes a BQC client program, which communicates with a completely independent entity that acts as a BQC server and programs and executes its own BQC server program).
We also note that programs often need to \textit{wait} for some event to happen (such as arrival of a message from a remote node), which presents a motivation for \textit{multitasking} (where waiting times may be filled by execution of other programs).
We refer back to \cref{chp:background} for more information about quantum network programs.
...
% The example above illustrates that quantum network programs consist of different types of operations.
