...
\addtocategory{noprint}{qnodeos_nature_noprint}
\addtocategory{noprint}{pompili_2022_experimental_noprint}
\addtocategory{noprint}{qoala_noprint}
\addtocategory{noprint}{qoala_compiler_noprint}
\addtocategory{noprint}{qne_sdk_noprint}
...
% Use "Listing" instead of "Example" when referring to code listings.
...
\lstdefinelanguage{netqasm}
{
    morekeywords={
        ...
        meas,
        create_epr, recv_epr,
        init, x, y, z, h, s, k, t, rot_x, rot_y, rot_z, cnot, cphase, crot_x, crot_y,
        APPID, NETQASM,
    }
    sensitive=false,
    morecomment=[l]{//},
...
\lstdefinelanguage{qoala}
{
    ...
    % morekeywords=[3]{
    %     parameters, name, csockets, epr_sockets,
    %     params, returns, uses, keeps, request,
    %     callback_type, callback, return_vars, remote_id, epr_socket_id, num_pairs, virt_ids, timeout, fidelity, typ, role
    % }
...
\newcommand{\steph}[1]{\textcolor{red}{STEPHANIE: #1}}
...
\acrodef{EHI}{Exposed Hardware Interface}
...
\acrodef{MLIR}{Multi-Level Intermediate Representation}
...
\acrodef{QAOA}{Quantum Approximate Optimization Algorithms}
...
\acrodef{VQE}{Variational Quantum Eigensolvers}
...
\include{chapters/main/netqasm/appendix}
\include{chapters/main/qnodeos/supplementary/source_code}
